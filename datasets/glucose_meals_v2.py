...
def normalize(df):
    ...
    for feature_name in df.columns:
        if feature_name == 'Date' or feature_name == 'id':
            continue
...
def split_by_individuals(cgm_file, meals_file, ratio=0.8):
    ...
    num_indiv_until = int(len(individuals) * 0.02)
    num_indiv = int(len(individuals) * ratio * 0.02)
    train_indiv = individuals[:num_indiv]
    val_indiv = individuals[num_indiv:num_indiv_until]
...
class GlucoseData(Dataset):
    ...
    def __init__(self, set_of, transform=None, Normalize=True):
        self.transform = transform
        cgm_df, meals_df = set_of

        self.cgm_df = cgm_df
        self.meals_df = meals_df

        if Normalize:
            self.cgm_df = normalize(self.cgm_df)
            self.meals_df.loc[self.meals_df.meal_type == 'meal', 'meal_type'] = 0
            self.meals_df.loc[self.meals_df.meal_type == 'snack', 'meal_type'] = 1
            self.meals_df.loc[self.meals_df.meal_type == 'drink', 'meal_type'] = 2
            self.meals_df.loc[self.meals_df.meal_type == 'restaurant', 'meal_type'] = 3
            self.meals_df = normalize(self.meals_df)
